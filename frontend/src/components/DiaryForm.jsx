  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 mb-8 transition-colors">
      <div className="relative">
        ...
        {preview && (
          <div className="absolute bottom-4 left-4 w-16 h-16 rounded-lg overflow-hidden border border-gray-200 shadow-sm group">
            ...
            <button aria-label="이미지 삭제" type="button" onClick={() => { setImage(null); setPreview(null); }} className="absolute inset-0 bg-black/50 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
              <FaTimes size={12} />
            </button>
          </div>
        )}
      ...
      <div className="flex justify-between items-center mt-4">
        <label className="cursor-pointer text-gray-500 hover:text-blue-500 transition p-2 rounded-full hover:bg-blue-50 dark:hover:bg-gray-700 focus-within:ring-2 focus-within:ring-blue-400">
          <FaImage size={20} />
          <input aria-label="사진 첨부" type="file" accept="image/*" onChange={handleImageChange} className="sr-only" disabled={isSubmitting} />
        </label>
        <button
          type="submit"
