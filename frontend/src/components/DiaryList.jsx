...
const DiaryList = ({ activeTab }) => {
  ...
  useEffect(() => {
    ...
    const initialFetch = async () => {
        try {
            ...
            else if (activeTab === 'home' && searchTerm) url = `/api/diaries/?all=true`;
            else url = `/api/diaries/?mode=calendar`; // 캘린더/통계: 경량화 데이터 로드
  ...
  const openModal = async (diary, startEditing = false) => {
    let fullDiary = diary;
    // content가 없으면 상세 조회 (캘린더/통계 모드에서 클릭 시)
    if (!diary.content && diary.id) {
        try {
            const response = await api.get(`/api/diaries/${diary.id}/`);
            fullDiary = response.data;
            // 로컬 상태 업데이트 (캐싱 효과)
            setDiaries(prev => prev.map(d => d.id === fullDiary.id ? fullDiary : d));
        } catch (error) {
            console.error("Failed to fetch diary details:", error);
            Swal.fire('오류', '일기 내용을 불러오는데 실패했습니다.', 'error');
            return;
        }
    }
